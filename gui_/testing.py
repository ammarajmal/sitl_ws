...
class ServerGUI(customtkinter.CTk):
    ...
    def start_data_recording(self):
        if not self.timer_running:
            ...
            duration = int(self.middle_frame_exp_duration_entry.get())
    ...
    def save_data_to_csv(self):
        ...
        with open(filename, mode='w', newline='') as file:
            ...
            writer.writerow(['Camera', 'Fiducial ID', 'X', 'Y', 'Z', 'Rot_X', 'Rot_Y', 'Rot_Z', 'Rot_W',
                             'Image Error', 'Object Error', 'Fiducial Area'])
            for camera, data in self.recorded_data.items():
                for row in data:
                    writer.writerow([camera] + row)
    ...
    def create_right_frame(self):
        ...
        self.right_frame.place(relx=(3 * gap) + (2 * frame_width), rely=0.13, relwidth=frame_width,
    ...
    def callback(self, data):
        ...
        for transform in data.transforms:
            print(f'Fiducial ID: {transform.fiducial_id}')
            ...
            recorded_data.append([
                ...
                transform.image_error,
                transform.object_error,
                transform.fiducial_area
            ])
        ...
        if self.timer_running and datetime.now() > self.end_time:
            if self.csv_check.get() == "on":
                self.save_data_to_csv()
            self.timer_running = False
